...
(function () {
  ...
  // ------------------------------------------------------------
  //  [중요] GitLab에서 최신 데이터를 직접 읽어옵니다.
  //  기존에는 이 브라우저의 localStorage만 읽고 있었는데, localStorage는
  //  PC/브라우저마다 따로 저장되는 값이라 - 데이터를 실제로 업로드한 그
  //  PC에서만 캐시가 남아있고, 다른 PC에서 챗봇을 열면 텅 빈 캐시만
  //  보여서 실적이 전부 0건으로 나오는 문제가 있었습니다.
  //  dashboard.html과 동일한 방식으로 GitLab의 data/measure,
  //  data/inspection 최신 버전을 동기적으로 직접 조회해서 이 문제를
  //  없앱니다. gitlab-storage.js가 이 페이지에 먼저 로드되어 있어야
  //  하며(<script src="gitlab-storage.js"> 를 chatbot-core.js보다
  //  앞에 두면 됨), 혹시 없거나 GitLab 접속이 실패하면 예전처럼
  //  localStorage 캐시로 자동 대체됩니다(오프라인 폴백).
  // ------------------------------------------------------------
